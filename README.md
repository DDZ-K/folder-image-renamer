...
| 模块 | 作用 |
|------|------|
| 图片命名 | 监视 `WatchRoot` 下一级子文件夹内图片；**静默等齐后一次性**复制到 `OutputRoot`，名为 `子文件夹_原文件名`；源文件不动；可按年/月/日归档 |
| DMC 缓存 | 每张成功拷贝入一条缓存：DMC = **改名后完整文件名（无扩展名）** |
| 云端放行 | 监视 log 目录；**文件名包含**缓存中的 DMC 即可（支持前缀）；内容 OK/NOK → 按配置键 |
| 归档 | OK / NOK / **超时**均归档；超时若无 log 会写 `__timeout` 占位文件 |
...
```text
监控目录\产品夹\ 多张图陆续落下
        ↓ 最后一张后再静默一段时间（FolderSettleMs）
输出目录\yyyy\MM\dd\  一次性出现 产品夹_图1.jpg、产品夹_图2.jpg …
        ↓ 缓存入 N 条 DMC（完整改名）
云端 → log 目录\前缀_完整DMC.txt  内容 OK / NOK
        ↓
模拟按键（默认 NumPad9 / NumPad7，可改 Home/PageUp）
        ↓
log 归档到 done\…__OK / __NOK / __timeout
```
...
1. 下载本仓库，或只拷贝发布目录（见 [Releases](../../releases) 若有，否则用本地 `publish` 构建产物）  
2. 进入 `publish\`（或你放置 exe 的目录）  
3. 双击 **`NgStationTool.exe`**  
4. **配置**里改成公司路径后，点 **开始**  
